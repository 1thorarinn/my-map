/* eslint-disable */
import React, { useRef, useState, useCallback, useEffect, memo, useMemo, Link } from 'react';
...
const HomePage = () => {
  ...
  const creationModeAction = (mapElement) => {

    console.log('routeId', routeId, mode)

    if (routeId === 0) {

      if (testing) console.log('Creation mode map action!')

      if (pinga.showCreationAdvisory()) {
            ...
            .then(res => {
              ...
              if (res.statusCode === 400) {
                ...
                if (isLineString) {
                  ...
                  console.log('setting routeId', res.id)
                  setRouteId(res.id)
                  setRouteName(name)
    ...
    loadMap: () => {
      ...
      if (map.current) {
        ...
        map.current.on('load', () => {
          ...
          map.current.on('move', (e) => { pinga.setMapOptions() })
          map.current.on('click', (e) => { pinga.editPlace(e, Draw.getSelected()) })

          map.current.on('draw.add', (e) => { pinga.updateRoute(e, Draw.getAll()) })
          map.current.on('draw.create', (e) => { pinga.updateRoute(e, Draw.getAll()) })
          map.current.on('draw.update', (e) => { pinga.updateRoute(e, Draw.getAll()) })
          map.current.on('draw.delete', (e) => { pinga.updateRoute(e, Draw.getAll()) })
          map.current.on('draw.select', (e) => { pinga.updateRoute(e, Draw.change()) })
      ...
      return (
        <div>
          ...
          <Select
            ...
            style={{ width: '97%' }}
            onChange={({ target: { value } }) => { pinga.loadRoute(value) }}>
          </Select>
        </div>
      )
    ...
    */
    ...
    editPlace: (e, selected) => {
      ...
      if (selected && selected.features.length > 0) {
        ...
        let placeUrl = placesOrigin + '?element=' + selected.features[0].id
        console.log(placeUrl)
        getData(placeUrl).then(data => {

          if (testing) console.log('RECOVERED')
          if (testing) console.log(data)

          pinga.resetPlace(false);
          setPlaceId(data[0].element)
    ...
    loadRoute: (selectedRouteId) => {
      ...
      if (selectedRouteId === 0) {
        ...
        fetch(routesOrigin + '/' + selectedRouteId)
          ...
          .then(response => {
            if (response) {
              pinga.resetToEditonMode(response)
            } else {
              pinga.resetToCreationMode()
            }
          })
    ...
    resetToCreationMode: () => {
      ...
      pinga.resetPublishButton()
      pinga.resetRouteDraw()
      setMode('creation')
      console.log('setting routeId', 0)
      setRouteId(0)
      setRouteName('')
      setRouteDescription('')
      setTimeout(() => { setIsLoading(false) }, 3000)
      pinga.flyTo(defaultCenter.lat, defaultCenter.lng, defaultCenter.zoom)
    },

    resetToEditonMode: (route) => {
      if (testing) console.log('EDITION MODE!::', routeId, route.id)
      pinga.resetRouteDraw()
      setMode('edition')
      console.log('setting routeId', route.id)
      setRouteId(route.id)
      setRouteName(route.name)
      setRouteDescription(route.description[0].description)
      setRouteIsPublished(route.published)
      pinga.drawSelectedRoute(route)
      pinga.flyTo(route.map_data.center_lat, route.map_data.center_long, route.map_data.center_zoom)
    ...
    updateRoute: (mapElement, data) => {
      var mode = getStorage('mode', 'string')
      if (mode === 'edition') {
        pinga.editionModeAction(mapElement)
      } else {
            ...
            <div className='row'>
              ...
              <div className='col-6' style={{ textAlign: 'center', marginTop: '20px' }}>
                <Button
                  label={'Cancel'}
                  color={'delete'}
                  onClick={pinga.cancelPlaceEdition}
                  className='my-button'
                />
              </div>
            </div>
  ...
  useEffect(pinga.loadMap, [routeId])
      ...
      <div className="row">
        ...
        <div className="col-md-4 col-lg-4">
          ...
          <div className='col-12'>
            <div className='row'>
              {pinga.renderRoutesSelector()}
              {pinga.renderInstructions()}
              {/*pinga.renderCarrousel()*/}
              <LoadingBar style={{ width: '100%', opacity: isLoading ? 99 : 0 }} />
              <div className="nav-bar">Mode: {mode} • routeId: {routeId}</div>
            </div>
          </div>
          <div className='col-12'>
